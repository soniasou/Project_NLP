...
def extract_embeddings_from_text(text):
    ...
    embeddings_padded = np.zeros((1, 1000))  # Create a vector of zeros of length 1000
    embeddings_padded[:, :embeddings_bag_of_word.shape[1]] = embeddings_bag_of_word  # Insert real embeddings
    return embeddings_padded.flatten().tolist()  # Return a vector of length 1000
def compute_text_embedding(text, model):
    text = text.lower().split()  # Convert text to a list of lowercase words
    embeddings = [model[word] for word in text if word in model]  # Get embeddings for each word in the template
    if embeddings:
        return np.mean(embeddings, axis=0).tolist()  # Average embeddings to obtain a single representation vector
    else:
        return np.zeros(model.vector_size).tolist()  # Return a zero vector if no words are present in the model
